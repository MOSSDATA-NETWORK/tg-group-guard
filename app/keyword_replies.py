...
# ===== 正则 ReDoS 防护 =====
# 用户配置的正则在消息热路径执行,标准库 re 无超时且回溯失控时持有 GIL,
# 线程池方案无法真正解救(事件循环照样饿死)。
# regex 库引擎对 (a+)+ / (x+x+)+ 等经典灾难模式有内建规避(实测微秒级返回),
# 并对残余病态模式提供原生 timeout 参数(超时抛 TimeoutError)。
# 超时/熔断按 pattern 文本记录,热重载后同名 pattern 仍保持禁用。
try:
...
if _regex_engine is None:
    logger.warning("未安装 regex 库(pip install regex),关键词正则将无超时防护")
...
async def safe_rule_match(rule: KeywordRule, text: str, timeout: float = _REGEX_TIMEOUT_SECONDS) -> bool:
    ...
    if _regex_engine is not None and isinstance(pattern, _regex_engine.Pattern):
        try:
            ...
            logger.warning(
                "关键词回复规则正则执行超时(>%.1fs),已熔断禁用 pattern=%r",
                timeout,
                pattern.pattern,
            )
            return False

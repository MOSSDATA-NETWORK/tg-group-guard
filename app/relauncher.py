...
用法(由 app.updater 内部调用,不需要手工执行):
    ...
    python app/relauncher.py --port 8000 [--timeout 60] [--delay 1.0] -- <实际启动命令...>
"""
...
def wait_for_port_free(port: int, timeout: float = 60.0, interval: float = 0.5) -> bool:
    """轮询直到可以成功 bind 该端口(父进程退出且残留连接被内核回收)。
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.port > 0:
        if wait_for_port_free(args.port, timeout=args.timeout):
            print(f"relauncher: 端口 {args.port} 已释放,启动新实例", flush=True)
        else:

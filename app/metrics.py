...
class PrometheusMetrics:
    ...
    def __init__(self) -> None:
        ...
        self.messages = Counter(
            "telegram_group_guard_bot_messages_total",
            "处理消息总数,按处理结果分桶",
            labelnames=("result",),
            registry=self.registry,
        )
        self.verifications = Counter(
            "telegram_group_guard_bot_verification_total",
            "验证流程结果",
            labelnames=("result",),
            registry=self.registry,
        )
        self.llm_latency = Histogram(
            "telegram_group_guard_bot_llm_latency_seconds",
            "LLM 调用延迟",
            labelnames=("provider",),
        ...
        self.llm_outcome = Counter(
            "telegram_group_guard_bot_llm_outcome_total",
            "LLM 调用结果",
            labelnames=("provider", "flagged"),
            registry=self.registry,
        )
        self.llm_in_flight = Gauge(
            "telegram_group_guard_bot_llm_in_flight",
            "正在进行的 LLM 调用数",
            registry=self.registry,
        )
        self.score_redis_degraded = Gauge(
            "telegram_group_guard_bot_score_redis_degraded",
